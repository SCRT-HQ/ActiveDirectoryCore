function Get-ADCoreConfig {
    <#
    .SYNOPSIS
    Loads the specified ADCore config
    ...
    This can be used to load either a legacy XML config from an older version of ADCore or a specific .PSD1 config created with version 2.0.0 or greater
    ...
    .EXAMPLE
    Get-ADCoreConfig personalDomain -PassThru
    ...
    Process {
        ...
        switch ($PSCmdlet.ParameterSetName) {
            ConfigurationModule {
                $fullConf = Import-SpecificConfiguration -CompanyName 'SCRT HQ' -Name 'ActiveDirectoryCore' -Scope $Script:ConfigScope
                if (!$ConfigName) {
        ...
        if (!$NoImport) {
            $Script:ActiveDirectoryCore = $decryptedConfig
        }
        if ($PassThru) {
            $decryptedConfig
        }
