function Set-ADCoreConfig {
    ...
    * User (this would create the config in the Local AppData folder for the user or it's *nix equivalent)
    ...
    .PARAMETER NoImport
    The default behavior when using Set-ADCoreConfig is that the new/updated config is imported as active. If -NoImport is passed, this saves the config but retains the previously loaded config as active.

    .EXAMPLE
    Set-ADCoreConfig -ConfigName "personal" -P12KeyPath C:\Keys\PersonalKey.p12 -AppEmail "myProjectName@myProject.iam.gserviceaccount.com" -AdminEmail "admin@domain.com" -CustomerID "C83030001" -Domain "domain.com" -Preference CustomerID -ServiceAccountClientID 1175798883298324983498 -SetAsDefaultConfig
    ...
    Process {
        ...
        $configHash = Import-SpecificConfiguration -CompanyName 'SCRT HQ' -Name 'ActiveDirectoryCore'
        if (!$ConfigName) {
    ...
    End {
        ...
        $configHash["$ConfigName"]['ConfigPath'] = (Join-Path $(Get-Module ActiveDirectoryCore | Get-ConfigurationPath -Scope $Script:ConfigScope) "Configuration.psd1")
        $configHash | Export-Configuration -CompanyName 'SCRT HQ' -Name 'ActiveDirectoryCore' -Scope $script:ConfigScope
        if (!$NoImport) {
            Get-ADCoreConfig -ConfigName $ConfigName -Verbose:$false
        }
    }
}
