function Get-ADCGroup {
    <#
    .SYNOPSIS
    Gets a directory group.

    .DESCRIPTION
    Gets a directory group.

    .PARAMETER Identity
    Specifies an Active Directory group by providing one of the following property values. The identifier in parentheses is the LDAP display name for the attribute. The acceptable values for this parameter are:
    ...
    * By using the domain of the computer running Windows PowerShell
    ...
    begin {
        try {
            ...
            $newparams['DefaultProperties'] = [ADGroup]::DefaultProperties
            $newparams['LdapFilter'] = switch ($PSCmdlet.ParameterSetName) {
                LdapFilter {
                    '(&(objectClass=group)(objectCategory=group){0})' -f $LdapFilter
                }
                Identity {
                    '(&(objectClass=group)(objectCategory=group){0})' -f ([ADCoreUtility]::ParseFilterFromIdentity($Identity))
                }
            }
